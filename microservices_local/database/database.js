...
async function findAllUsers() {
  return new Promise((resolve, reject) => {
    db.all(`SELECT firstname, lastname, mail, role
            FROM user`, (err, rows) => {
      if (err) {
...
async function updateUserRoleByMail(mail, newRole) {
  return new Promise((resolve, reject) => {
    db.run(`UPDATE user
            SET role = ?
            WHERE mail = ?`, [newRole, mail], function (err) {
      if (err) {
...
async function addReservation(reservation) {
  ...
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO reservation (userId, courtId, date, startHour)
       VALUES (?, ?, ?, ?)`,
...
async function deleteReservation(reservationId) {
  return new Promise((resolve, reject) => {
    ...
    db.get(`SELECT *
            FROM reservation
            WHERE id = ?`, [reservationId], (err, row) => {
      if (err) {
      ...
      db.run(`DELETE
              FROM reservation
              WHERE id = ?`, [reservationId], function (err) {
        if (err) {
...
async function startWatering(courtId) {
  ...
  return new Promise((resolve, reject) => {
    db.run(`UPDATE court
            SET lastWatering = ?,
                running      = true
            WHERE id = ?`, [newDate, courtId], function (err) {
      if (err) {
